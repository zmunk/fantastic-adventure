...
class Canvas(QLabel):
    ...
    def drawFace(self, points, brightness=None, color=120):
        ...
        p.setBrush(QBrush(QColor.fromHsv(color, 255, shade, 255), Qt.SolidPattern))
...
class Vector(object):
    ...
    def toRotationQuaternion(self, half_angle):
        return Quaternion(cos(half_angle), self.x * sin(half_angle), self.y * sin(half_angle), self.z * sin(half_angle))
...
class Face(object):
    ...
    def draw(self, canv, cube_center, color):
        ...
        for point in self.points:
            pts.append(point.perspectivepoint(canv))  # each point is an instance of Point2D
...
class Point(object):
    ...
    def reset(self):
        ...
        self.rotation_quaternion = Quaternion(1, 0, 0, 0)  # initial identity quaternion
